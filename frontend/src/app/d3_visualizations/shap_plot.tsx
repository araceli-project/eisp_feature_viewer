...
import type { TrainResponse } from "../services/trainService";
import type { FeatureData } from "./feature_data";
...
export function renderShapPlot(
  ...
  const margin = { top: 32, right: 36, bottom: 56, left: 180 };
...
export function renderShapPlotSelectedPoints(
  ...
  const margin = { top: 32, right: 36, bottom: 56, left: 140 };
  const width = options.width - margin.left - margin.right;
  const height = options.height - margin.top - margin.bottom;
  ...
  for (const [feature, values] of Object.entries(featureData.shap_results)) {
    filteredShap_results[feature] = selectedPointIndices.map(
      (index) => values[index],
    );
  }
  console.log("Data for SHAP plot of selected points:", filteredShap_results);

  const data_per_feature = Object.entries(filteredShap_results).map(
    ([feature, values]) => ({
      feature,
      importance:
        values.reduce((a, b) => Math.abs(a) + Math.abs(b), 0) / values.length,
    }),
  );

  const maxValueFromAllPoints =
    d3.max(Object.values(featureData.shap_results).flat(), (d) =>
      Math.abs(d),
    ) || 0;
  const maxValueFromSelectedPoints =
    d3.max(data_per_feature, (d) => d.importance) || 0;
  const domainMax =
    selectedPointIndices.length ===
    featureData.shap_results[Object.keys(featureData.shap_results)[0]].length
